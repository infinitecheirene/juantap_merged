...
import {
  ...
  Linkedin, Github, Youtube, QrCode, Share2, Download, MapPin, Phone, User as UserIcon
} from "lucide-react";
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "@/components/ui/dialog";
import { Button } from "@/components/ui/button";
import { QRCodeSVG } from "qrcode.react";
import type { Template, User, SocialLink } from "@/types/template";
...
export const PreviewRenderer: React.FC<PreviewRendererProps> = ({ template, user, slug }) => {
  ...
  const profileUrl = `${window.location.origin}/${user?.username || slug}`;
...
END:VCARD
  ...
  return (
    <div className="w-full min-h-screen flex justify-center px-4 py-6 sm:px-6 bg-background">
      <div className="w-full max-w-lg shadow-2xl rounded-2xl overflow-hidden flex flex-col mx-auto animate-in fade-in duration-500 bg-card"
        style={{
      ...
      >
        <div className="relative flex flex-col items-center pt-8 px-4 sm:px-6">
          ...
          <div className="w-full px-6 py-3 bg-primary">
            <h1
              className="text-lg sm:text-xl font-bold text-center text-primary-foreground uppercase tracking-wide"
              style={{
                fontFamily: template?.fonts?.heading || 'system-ui',
              }}
            >
          ...
          {user?.profile?.bio && (
            <p
              className="text-sm text-center mt-4 px-2 leading-relaxed text-muted-foreground"
              style={{
                fontFamily: template?.fonts?.body || 'system-ui',
              }}
            >
        ...
        <div className="p-6 space-y-2">
          ...
          {user?.email && (
            <div 
              className="flex items-center gap-3 rounded-lg p-3 text-sm transition-colors hover:bg-muted border border-border"
              style={{
                fontFamily: template?.fonts?.body || 'system-ui'
              }}
            >
              ...
              <div className="flex flex-col gap-2 flex-1 min-w-0">
                {user.email.split(',').map((email, index) => {
                  ...
                  return (
                    <div
                      key={index}
                      className="flex justify-between items-center gap-2 text-foreground"
                    >
                      <a
                        href={`mailto:${trimmed}`}
    ...
    <div className="flex flex-col gap-1.5 flex-1 min-w-0">
      {user.profile.phone.split(',').map((phone, index) => {
       ...
       {user?.profile?.socialLinks && user.profile.socialLinks.length > 0 && (
                  ...
                  .map((link: SocialLink) => {
                    ...
                    return (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="flex items-center justify-center rounded-xl w-10 h-10 sm:w-12 sm:h-12 flex-shrink-0 hover:scale-110 transition-all hover:opacity-80 shadow-md active:scale-95"
                        style={{ backgroundColor: brandColor }}
                        title={link.platform}
                      >
                        <div className="scale-[0.7] sm:scale-75">
                          {getSocialIconWhite(platformKey)}
                        </div>
                      </a>
                    );
                  })}
        ...
        <div
          className="flex justify-around border-t border-border p-4 bg-card"
          style={{
            fontFamily: template?.fonts?.body || 'system-ui',
          }}
        >
          <button
            ...
            className="flex flex-col items-center text-sm hover:scale-110 transition-transform text-foreground"
          >
            <QrCode className="w-5 h-5 mb-1 text-muted-foreground" />
          ...
          <button
            ...
            className="flex flex-col items-center text-sm hover:scale-110 transition-transform text-foreground"
          >
            <Share2 className="w-5 h-5 mb-1 text-muted-foreground" />
          ...
          <div className="flex flex-col items-center space-y-4 p-2">
            ...
            <div className="w-full p-3 bg-muted rounded-lg">
              ...
              <div className="flex items-center justify-between gap-2 w-full">
                <code className="text-[10px] sm:text-xs text-foreground truncate flex-1 min-w-0 break-all">
                  {profileUrl}
                </code>
                <Button
                  variant="ghost"
